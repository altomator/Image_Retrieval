...
def save_palette_as_image(filename, palette):
    ...
    if palette.bgcolor is None:
        size = (PALETTE * len(palette.colors), PALETTE)
    else:
        size = (PALETTE * (len(palette.colors)+1), PALETTE)
    im = Image.new('RGB', size)
    draw = ImageDraw.Draw(im)
    for i, c in enumerate(palette.colors):
        v = colorsys.rgb_to_hsv(*norm_color(c.value))[2]
        (x1, y1) = (i * PALETTE, 0)
        (x2, y2) = ((i + 1) * PALETTE - 1, PALETTE-1)
    ...
    if not palette.bgcolor is None:
        (x1, y1) = (len(palette.colors)* PALETTE, 0)
        (x2, y2) = ((len(palette.colors)+1) * PALETTE - 1, PALETTE-1)
        v = colorsys.rgb_to_hsv(*norm_color(palette.bgcolor.value))[2]
        if v < 0.4: # grey outline
            draw.rectangle([(x1, y1), (x2, y2)], fill=(200,200,200))
        else: # black
            draw.rectangle([(x1, y1), (x2, y2)], fill=(0,0,0))
        draw.rectangle([(x1+1, y1+1), (x2-1, y2-1)], fill=(palette.bgcolor.value))
...
#####
print("max colors to be detected: ",config.MAX_COLORS)
print("palette:%f ",PALETTE)
print("min saturation:%f ",config.MIN_SATURATION)
print("min prominence:%f",config.MIN_PROMINENCE)
print("min distance:%f",config.MIN_DISTANCE)
print("bckg prominence:%f",config.BACKGROUND_PROMINENCE)
color_stream_st()
