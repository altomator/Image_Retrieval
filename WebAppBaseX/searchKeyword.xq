(:
 Recherche de pages illustrees par le texte (OCR)
:)
...
(: requete :)
declare variable $keyword as xs:string external := "voiture";
...
declare variable $toDate as xs:string external := "1922-12-31";  (: par defaut toute la collection :)
...
<h2>Annotations (yolo v4) : <span  style="color:#990000">{$keyword}</span></h2>
<h3><b>Base</b> : {$corpus} </h3>
<h3><b>Pages indexées</b> : {count($data//page)} </h3>
<h3><b>Illustrations</b> :  {count($data//ill)}
de  {$fromDate} à la page {$toDate} </h3>
<ol>
  {
  ...
  (metad/dateEdition/text() gt $fromDate)
  and (metad/dateEdition/text() lt $toDate)]
...
(: ### REQUETE ### :)
for $ill in $page/ills/ill[(titraille contains text {$keyword} any ) or (leg contains text {$keyword} any ) or (txt contains text {$keyword} any)]
