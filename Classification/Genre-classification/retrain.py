...
def maybe_download_and_extract():
  ...
  if not os.path.exists(filepath):
    ...
    filepath, _ = req.urlretrieve(DATA_URL, filepath, _progress)
    print()
...
def main(_):
  ...
  proxy = req.ProxyHandler({'http': 'http://wwwcache.univ-lr.fr:3128', 'https': 'https://wwwcache.univ-lr.fr:3128'})
...
############# Script pour d'entrainement du modèle #############
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--output_graph',
      type=str,
      default= outputDir + r'/output_graph.pb',
      help='Where to save the trained graph.'
  )
  parser.add_argument(
      '--output_labels',
      type=str,
      default= outputDir + r'/output_labels.txt',
      help='Where to save the trained graph\'s labels.'
  )
  parser.add_argument(
      '--summaries_dir',
      type=str,
      default= outputDir + r'/retrain_logs',
  ...
  parser.add_argument(
      '--bottleneck_dir',
      type=str,
      default= outputDir + r'/bottleneck',
